...
CLI Mode
--------
- Run `python src/main.py --goal "..."` (LangGraph is the default; legacy is only used as fallback).
- If no goal is provided, the app prompts for a goal and keeps the browser open.
- `--clean-between-goals` wipes logs/state/screenshots between goals (profile intact).
- `INTERACTIVE_PROMPTS` controls whether the agent asks questions (default off).
...
- `--ui-step-limit` can override max_steps for UI shell runs only.

Confirmations & Prompts
-----------------------
- Safety confirm: triggered on destructive heuristics or risky navigation; `--auto-confirm` bypasses the prompt.
- Progress ask_user: only on later stages (locate/verify); in non-interactive mode sets stop_reason without blocking; in interactive mode asks the user.
- Ask_user node: same interactive/non-interactive behavior controlled by INTERACTIVE_PROMPTS.

Flags Commonly Used
-------------------
- `--plan-only` / `--auto-confirm`
- `--langgraph` (deprecated; LangGraph is always on by default, legacy only on fallback)
...
- If INTERACTIVE_PROMPTS=false, the agent will not wait for terminal input (except safety confirm unless auto_confirm).
